...
class Tabela:
    ...
    def sequencialRecursion(self,chave,tamanho):
        if(not self.vazia()):
            if(self.chave[tamanho] == chave):
                return tamanho
            else:
                return self.sequencialRecursion(chave,tamanho+1)
        return -1
        
    def binariaRecursion(self,vet,chave, esq, dire):
        meio = ((esq + dire)//2)
        aux_chave = vet[meio]
        if chave == aux_chave:
            return meio
        elif chave > aux_chave:
            return self.binariaRecursion(vet, chave, meio, dire)
        return self.binariaRecursion(vet, chave, esq, meio)
    ...
    def consulta(self,chave,op):
        ...
        if (op == 'seq'):
            ...
            if(posicao > -1):
                ...
                return 0,0
        if (op == 'seqRec'):
            a = self.sequencialRecursion(chave,0)
            return self.valor[a]
        if (op == 'binRec'):
            vet = self.chave
            a = self.binariaRecursion(vet,chave,0,self.tamanho)
            return self.valor[a]
