...
class LinkedList:
    ...
    def getNodeRecursion(self, index, tamnho, auxPointer):   
        if (index > tamnho):
            if auxPointer:
                auxPointer = auxPointer.next
                return self.getNodeRecursion(index, tamnho+1, auxPointer)
            else:
                raise IndexError("Lista fora de range")
        return auxPointer
        
    def searchRecursion(self, elemt,auxPointer,i):
        if auxPointer == None:
            return -1
        elif auxPointer.data == elemt:
            return i
        else:
            auxPointer = auxPointer.next
            i = i + 1
            return self.searchRecursion(elemt,auxPointer,i)
        return -1
    ...
    def __getitem__(self, index):                    #sobrecarga de operador, ou seja, trata a lista encadeada como uma lista de vetores, sequencialmente.
